...
from scm.scenario import BASE_LEAD_TIME_DAYS, diagnose_lead_time_scenario
...
def sync_scenario_from_lead_time_input() -> None:
    """실제 예상 납기 입력값을 현재 계산 시나리오에 반영한다."""

    lead_time_days = st.session_state.get("actual_lead_time_days")
    if lead_time_days is None:
        return
    try:
        diagnosis = diagnose_lead_time_scenario(float(lead_time_days))
    except (TypeError, ValueError):
        return
    st.session_state["selected_scenario"] = diagnosis.scenario
...
if st.session_state.get("selected_scenario") not in scenario_keys:
    st.session_state["selected_scenario"] = "moderate"
if "actual_lead_time_days" not in st.session_state:
    default_multiplier = float(SCENARIOS[st.session_state["selected_scenario"]]["multiplier"])
    st.session_state["actual_lead_time_days"] = round(BASE_LEAD_TIME_DAYS * default_multiplier)

scenario = st.sidebar.selectbox(
    "물류 상황",
    options=scenario_keys,
    format_func=lambda key: f"{SCENARIOS[key]['label']} · {SCENARIOS[key]['multiplier']}배",
    index=scenario_keys.index(st.session_state["selected_scenario"]),
...
if page == "한눈에 보기":
    ...
    render_hero(
        ...
        "실제 예상 납기와 물류 시나리오를 연결해 현재 계산 조건을 확인합니다.",
    )
    render_flow_note(
        "3 / 8단계: 위험 시나리오 선택",
        "시나리오를 직접 선택하거나 실제 납기를 입력하면 충격 수준을 자동으로 진단합니다.",
    )
    st.info(
    ...
    lead_time_days = st.number_input(
        ...
        step=1.0,
        format="%.0f",
        key="actual_lead_time_days",
        on_change=sync_scenario_from_lead_time_input,
        help=f"기준 리드타임은 World Bank 2023 평균 리드타임 {BASE_LEAD_TIME_DAYS:.0f}일입니다.",
    )
    lead_time_diagnosis = diagnose_lead_time_scenario(float(lead_time_days))
    diagnosed_label = str(SCENARIOS[lead_time_diagnosis.scenario]["label"])
    st.success(
        f"입력 납기 {lead_time_days:.0f}일 ÷ 기준 {BASE_LEAD_TIME_DAYS:.0f}일 = "
        f"{lead_time_diagnosis.multiplier:.2f}배 · {lead_time_diagnosis.message} "
        f"({diagnosed_label})"
    )
    ...
    for column, (key, details) in zip(scenario_columns, SCENARIOS.items(), strict=True):
        with column:
            with st.container(border=True):
                ...
                recommended_mark = " · 납기 진단" if lead_time_diagnosis.scenario == key else ""
    ...
    with st.expander("기존 우선순위 수량 배분도 함께 보기"):
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "name": "품목명",
                "abc_xyz": "관리등급",
                "required_budget": st.column_config.NumberColumn("필요예산", format="₩%.0f"),
                "allocated_budget": st.column_config.NumberColumn("배정예산", format="₩%.0f"),
                "allocated_quantity": "배정수량",
                "unmet_quantity": "미충족수량",
                "fulfillment_rate": st.column_config.NumberColumn("충족률", format="percent"),
            },
        )
    render_limitations()
    ...
    if api_key:
        st.success(f"OpenAI API 연결 완료 · 모델: {openai_model}")
    else:
        st.info(
            "OpenAI API 키가 없어 데모 조언을 사용합니다. 배포 환경의 Secrets에 "
            "`OPENAI_API_KEY`를 설정하면 실제 호출을 사용할 수 있습니다."
        )
    ...
    st.markdown(
        ...
        "2. 위험 시나리오에서 실제 납기 입력값과 사용자가 선택한 시나리오를 비교합니다.\n"
    ...
    for key, details in SCENARIOS.items():
        ...
        if key in {"high", "extreme"}:
            st.markdown(f"- **{details['label']} {details['multiplier']}배** — {source_text}")
            continue
        st.markdown(
